...
class BbanksaPipeline:

    # Database setup
    conn = sqlite3.connect('banksa.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `banksa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM banksa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO banksa (date, title, link, content)"
